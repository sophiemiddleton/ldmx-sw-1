...
namespace ldmx {
    ...
    const double HcalDigiProducer::ELECTRONS_PER_MIP = 33000.0; // e-
    const double HcalDigiProducer::CLOCK_CYCLE = 25.0; // ns
    const double HcalDigiProducer::MIP_SI_RESPONSE = 0.130; // MeV
    const int    HcalDigiProducer::NUM_ECAL_LAYERS = 34;
    const int    HcalDigiProducer::NUM_HEX_MODULES_PER_LAYER = 7;
    const int    HcalDigiProducer::CELLS_PER_HEX_MODULE = 397;
    const int    HcalDigiProducer::TOTAL_NUM_CHANNELS = NUM_ECAL_LAYERS*NUM_HEX_MODULES_PER_LAYER*CELLS_PER_HEX_MODULE;
	...
	mev_per_mip_     = ps.getParameter<double>("mev_per_mip");
        ...
        pulseFunc_ = TF1(
                ...
                0.0,(double) nADCs_*HcalDigiProducer::CLOCK_CYCLE
                );
	...
	//Loop over sim hits
        ...
        for (auto const& simHit : hcalSimHits ) {
            ...
            std::vector<HcalDigiSample> digiToAdd;
        ...
        for ( double noiseHit : noiseHitAmplitudes ) {
            ...
            double hitTime = noiseInjector_->Uniform( HcalDigiProducer::CLOCK_CYCLE );
    ...
    bool HcalDigiProducer::constructDigis(
	    ...
	    double meanPEInWindow = 0.0;
	      ...
	      meanPEInWindow = (energyInWindow / mev_per_mip_) * pe_per_mip_;
            ...
            timeInWindow   += noiseInjector_->Gaus( 0.0 , HcalDigiProducer::CLOCK_CYCLE / 100. ); 
            ...
            if ( pulseFunc_.Eval( -nADCs_*HcalDigiProducer::CLOCK_CYCLE ) > readoutThreshold_ ) 
                toa = -nADCs_*HcalDigiProducer::CLOCK_CYCLE; //always above --> toa is beginning of readout interval
            else
                toa = pulseFunc_.GetX(readoutThreshold_, -nADCs_*HcalDigiProducer::CLOCK_CYCLE, timeInWindow);

            double tut(0.);
            // check if second half is just always above readout
            if ( pulseFunc_.Eval( nADCs_*HcalDigiProducer::CLOCK_CYCLE ) > readoutThreshold_ )
                tut = nADCs_*HcalDigiProducer::CLOCK_CYCLE; //always above --> tut is end of readout interval
            else
            ...
            digiToAdd[iSOI_].toa_     = toa * (1024/HcalDigiProducer::CLOCK_CYCLE); //conversion from ns to clock counts
