/**
 * @file HcalDigiCollection.cxx
 * @author Tom Eichlersmith, University of Minnesota
 */

#include "Event/HcalDigiCollection.h"

ClassImp(ldmx::HcalDigiCollection)

namespace ldmx {

    void HcalDigiCollection::Clear() {
    ...
    void HcalDigiCollection::Print() const {

        std::cout << "HcalDigiCollection { Num Channel IDs: " << channelIDs_.size()
    ...
    std::vector< HcalDigiSample > HcalDigiCollection::getDigi( unsigned int digiIndex ) const {
        
        std::vector< HcalDigiSample > digi;
        for ( unsigned int sampleIndex = 0; sampleIndex < this->getNumSamplesPerDigi(); sampleIndex++ ) {
    
            HcalDigiSample sample;
    ...
    void HcalDigiCollection::addDigi( std::vector< HcalDigiSample > newSamples ) {

        if ( newSamples.size() != this->getNumSamplesPerDigi() ) {
            std::cerr << "[ WARN ] [ HcalDigiCollection ] Input list of samples has size '"
